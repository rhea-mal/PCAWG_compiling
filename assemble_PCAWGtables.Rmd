...
listfile <- list.files("/Users/rhea/Desktop/summer/consensus_cnv/consensus.20170119.somatic.cna.icgc.public",pattern = "txt",full.names = T, recursive = TRUE)
...
for (i in 1:length(listfile)){
  if(i==1){
    ...
    name <- str_replace(name, "/Users/rhea/Desktop/summer/consensus_cnv/consensus.20170119.somatic.cna.icgc.public/", "")
    df[ , 'aliquot_id'] <- name
  }
    ...
    newname <- str_replace(newname, "/Users/rhea/Desktop/summer/consensus_cnv/consensus.20170119.somatic.cna.icgc.public/", "")
...
write.csv(breast,"/Users/rhea/Desktop/PCAWGbreastsamples.csv", row.names = FALSE)
```
...
breast <- joined[joined$dcc_project_code == 'BRCA-UK', ]
breast <- na.omit(breast)

#ovarian <- joined[joined$dcc_project_code == 'OV-AU', ]
...
breast2 <- data.frame(matrix(ncol = 5, nrow = 0))
colnames(breast2) <- c('chr', 'start', 'end', 'copy', 'ploidy')
...
for (i in 1:nrow(breast)){
  strt <- breast$start[i]
  while (breast$end[i] - strt >= 500000) {
  bin <- ceiling(strt/500000) * 500000
  chr <- c(breast$chromosome[i])
  start <- c(strt)
  end <- c(bin)
  copy <- c(breast$total_cn[i])
  ploidy <- c(breast$ploidy[i])
  new_data <- data.frame(chr, start, end, copy, ploidy)
  breast2 <- rbind(breast2, new_data)
  strt <- bin + 1
  }
  chr <- c(breast$chromosome[i])
  start <- c(strt)
...
breast <- inner_join(breast, ploidy, by="aliquot_id")
